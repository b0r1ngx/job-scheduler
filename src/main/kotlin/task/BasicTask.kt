...
open class BasicTask(
    ...
    override val executionTime: Long = 3000,
    final override val suspendingTime: Long = 1000
) : Task {
    ...
    override fun start() {
        if (state == State.READY) {
            state = State.RUNNING
            println("PROCESSOR: executing task - $this")
        } else {
    ...
    override fun terminate() {
        if (state == State.RUNNING) {
            ...
            println("PROCESSOR: executed task -  $this")
        } else {
            throw Exception("Illegal state of the task $name: state was $state but must be RUNNING")
        }
    }
